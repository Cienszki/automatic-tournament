 ...
 */
async function closeOrphanedLobbies(db, runners, busyAccounts, isShuttingDown) {
    if (isShuttingDown())
        return;
    let games;
    try {
        ...
        logger_1.logger.error('[Conductor] Failed to scan for orphaned lobbies', error);
        return;
    }
    const now = Date.now();
    for (const game of games) {
        if (!(0, types_1.isTerminal)(game.state) || !game.dotaLobbyId || !game.botAccountId)
            continue;
        const endedAt = Date.parse(game.endedAt ?? game.updatedAt);
        if (!Number.isFinite(endedAt) || now - endedAt > ORPHAN_LOBBY_MAX_AGE_MS)
            continue;
        ...
        if (existing?.child && existing.child.exitCode === null)
            continue;
        logger_1.logger.warn(`[Conductor] Game ${game.id} is '${game.state}' but still holds lobby ${game.dotaLobbyId} — ` +
