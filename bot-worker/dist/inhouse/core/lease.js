...
exports.LEASE_TIMEOUT_MS = void 0;
exports.leaseAccount = leaseAccount;
exports.renewLease = renewLease;
exports.releaseAccount = releaseAccount;
 ...
 */
exports.LEASE_TIMEOUT_MS = 3 * 60_000;
/**
...
async function leaseAccount(db, gameId) {
    ...
    for (const candidate of snapshot.docs) {
        const claimed = await db.runTransaction(async (tx) => {
            ...
            const heartbeatFresh = data.leaseHeartbeatAt !== null &&
            ...
            if (heldByAnother && heartbeatFresh)
                return false;
            ...
            if (!reclaimable && data.status !== 'idle')
                return false;
            ...
            // unusable.
            if (data.cooldownUntil && Date.parse(data.cooldownUntil) > Date.now())
                return false;
        ...
        if (claimed) {
            logger_1.logger.info(`Leased bot account ${candidate.id} to game ${gameId}`);
            return { ok: true, botAccountId: candidate.id };
        }
    }
    logger_1.logger.warn(`No bot account available for game ${gameId}`);
    return { ok: false, reason: 'none_available' };
}
/** Renew a lease. Called by the worker heartbeat. */
async function renewLease(db, botAccountId, gameId) {
...
async function releaseAccount(db, botAccountId) {
    try {
        await db.collection(BOT_ACCOUNTS).doc(botAccountId).update({
            status: 'idle',
            leasedByGameId: null,
        ...
        logger_1.logger.info(`Released bot account ${botAccountId}`);
    }
    catch (error) {
        logger_1.logger.warn(`Failed to release bot account ${botAccountId}: ${String(error)}`);
    }
}
/**
 ...
 */
async function poolStatus(db) {
    const snapshot = await db.collection(BOT_ACCOUNTS).where('enabled', '==', true).get();
    const cutoff = Date.now() - exports.LEASE_TIMEOUT_MS;
    const leased = snapshot.docs.filter((doc) => {
        const data = doc.data();
        return (Boolean(data.leasedByGameId) &&
