...
exports.CHANGEABLE_SETTINGS = exports.TOURNAMENT_SETTINGS = exports.DEFAULT_SETTINGS = exports.LOBBY_VISIBILITY = exports.DOTA_TV_DELAYS = exports.SERVER_REGION_NAMES = exports.SERVER_REGIONS = exports.GAME_MODE_NAMES = exports.GAME_MODES = void 0;
exports.lobbyVisibilityFor = lobbyVisibilityFor;
exports.resolveSettings = resolveSettings;
exports.normalizeDotaTvDelay = normalizeDotaTvDelay;
exports.parseSettingCommand = parseSettingCommand;
...
exports.DOTA_TV_DELAYS = [10, 120, 300, 900];
/** DOTALobbyVisibility. */
exports.LOBBY_VISIBILITY = {
    public: 0,
    friends: 1,
    unlisted: 2,
};
 ...
 */
function lobbyVisibilityFor(published) {
    return published ? exports.LOBBY_VISIBILITY.public : exports.LOBBY_VISIBILITY.unlisted;
}
// ─── Defaults ────────────────────────────────────────────────────────────────
/**
...
exports.DEFAULT_SETTINGS = {
    ...
    published: false,
    locked: false,
    gameMode: exports.GAME_MODES.ap,
    serverRegion: exports.SERVER_REGIONS.eueast,
    dotaTvDelay: 120,
    leagueId: 0,
...
exports.TOURNAMENT_SETTINGS = {
    ...exports.DEFAULT_SETTINGS,
    mode: 'tournament',
    gameMode: exports.GAME_MODES.cm,
    published: false,
    locked: false,
    reservationTtlSeconds: 0,
    autoPublishNudgeMinutes: 0,
};
function baselineFor(mode) {
    return mode === 'tournament' ? exports.TOURNAMENT_SETTINGS : exports.DEFAULT_SETTINGS;
}
// ─── Coercion helpers ────────────────────────────────────────────────────────
...
function str(v, allowed, fallback) {
    if (typeof v === 'string' && allowed.includes(v))
        return v;
    return fallback;
}
function numArray(v, fallback) {
...
function resolveSettings(...layers) {
    const present = layers.filter((l) => l !== undefined && l !== null);
    let mode = exports.DEFAULT_SETTINGS.mode;
...
function normalizeDotaTvDelay(seconds) {
    if (seconds <= 10)
        return 10;
    if (seconds <= 120)
        return 120;
 ...
 */
function parseSettingCommand(key, rawArg) {
    const arg = rawArg.trim().toLowerCase();
    switch (key) {
        case 'mode': {
            if (!arg)
                return { ok: false, message: `Usage: !mode ap|cm|rd|sd|ar|turbo` };
            const value = exports.GAME_MODES[arg];
            if (value === undefined) {
                return { ok: false, message: `Unknown mode "${rawArg.trim()}". Try: ap, cm, rd, sd, ar, turbo` };
            }
            return { ok: true, patch: { gameMode: value }, message: `Mode set to ${exports.GAME_MODE_NAMES[value] ?? value}.` };
        }
        case 'region': {
            if (!arg)
                return { ok: false, message: `Usage: !region eu|eueast|useast|sea|…` };
            const value = exports.SERVER_REGIONS[arg.replace(/[\s_-]/g, '')];
            if (value === undefined) {
                return { ok: false, message: `Unknown region "${rawArg.trim()}". Try: eu, eueast, useast, uswest, sea, aus` };
            }
            return { ok: true, patch: { serverRegion: value }, message: `Region set to ${exports.SERVER_REGION_NAMES[value] ?? value}.` };
        }
        case 'delay': {
            if (!arg)
                return { ok: false, message: `Usage: !delay 0|2|5|15  (minutes)` };
...
function formatSettings(s) {
    const parts = [
        exports.GAME_MODE_NAMES[s.gameMode] ?? `mode ${s.gameMode}`,
        exports.SERVER_REGION_NAMES[s.serverRegion] ?? `region ${s.serverRegion}`,
        `DotaTV ${s.dotaTvDelay >= 60 ? `${s.dotaTvDelay / 60}m` : `${s.dotaTvDelay}s`}`,
        `first pick ${s.selectionPriorityRules === 1 ? 'coin flip' : 'manual'}`,
    ];
    if (s.immortalDraft)
        parts.push('immortal draft');
    if (s.locked)
        parts.push('LOCKED');
    return parts.join(' · ');
}
/** What `!help settings` lists. */
exports.CHANGEABLE_SETTINGS = ['mode', 'region', 'delay', 'immortal', 'firstpick'];
