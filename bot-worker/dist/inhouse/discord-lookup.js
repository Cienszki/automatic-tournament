...
exports.findGuildMember = findGuildMember;
 ...
 */
async function findGuildMember(cfg, query) {
    ...
    let members;
    try {
        const url = `${DISCORD_API}/guilds/${cfg.guildId}/members/search` +
        ...
        if (res.status === 403) {
            logger_1.logger.error('[InhouseRunner] Discord member search returned 403 — the SERVER MEMBERS INTENT is ' +
                'probably not enabled for this bot (Developer Portal → Bot → Privileged Gateway Intents).');
            return { ok: false, reason: 'error' };
        }
        if (res.status === 401) {
            logger_1.logger.error('[InhouseRunner] Discord rejected the bot token (401) — check DISCORD_TOKEN.');
            return { ok: false, reason: 'error' };
        }
        if (!res.ok) {
            logger_1.logger.warn(`[InhouseRunner] Discord member search failed: HTTP ${res.status}`);
            return { ok: false, reason: 'error' };
        }
        members = (await res.json());
    }
    catch (error) {
        logger_1.logger.warn(`[InhouseRunner] Discord member search could not be reached: ${String(error)}`);
        return { ok: false, reason: 'error' };
    }
    ...
    const nameOf = (m) => displayNameOf(m).toLowerCase();
    const userOf = (m) => (m.user?.username ?? '').toLowerCase();
    const tiers = [
