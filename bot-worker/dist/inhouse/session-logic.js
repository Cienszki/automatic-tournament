...
exports.InhouseSessionLogic = void 0;
...
class InhouseSessionLogic {
    ...
    stop() {
        if (this.hostAnnounceTimer)
            clearTimeout(this.hostAnnounceTimer);
    ...
    async onLobbyUpdate(players) {
        ...
        try {
            ...
            logger_1.logger.error(`[InhouseRunner] Lobby sync failed for game ${this.game.id}`, error);
        }
        finally {
            this.syncing = false;
        }
    ...
    async publishSlotState() {
        ...
        const slots = store_1.InhouseStore.computeSlots(present, reservations);
        const snapshot = {
        ...
        const identityFingerprint = JSON.stringify(present.map((m) => [m.steamId32, m.side, m.displayName ?? '', m.playerName ?? ''].join(' ')).sort());
    ...
    async reconcileHost() {
        ...
        const playing = allMembers.filter((m) => m.leftAt === null && types_1.PLAYING_SIDES.includes(m.side));
        if (hostSteamId !== null) {
            if (playing.some((m) => m.steamId32 === hostSteamId))
                return; // still seated — a side swap included
            const everSeen = allMembers.some((m) => m.steamId32 === hostSteamId);
            if (!everSeen)
                return; // named host hasn't walked in yet — not a departure
        }
     ...
     */
    scheduleHostAnnounce(steamId32, name) {
        this.clearHostAnnounce();
        this.hostAnnounceTimer = setTimeout(() => {
            this.hostAnnounceTimer = null;
            if (this.game.initiatorSteamId32 !== steamId32)
                return;
            void this.deps
...
exports.InhouseSessionLogic = InhouseSessionLogic;
...
function mapSide(team) {
    if (team === 'radiant' || team === 'dire' || team === 'spectator' || team === 'unassigned')
        return team;
    return 'unassigned';
}
