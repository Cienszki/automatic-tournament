...
import { InhouseStore } from '../inhouse/core/store';
import type { InhouseGame, Membership } from '../inhouse/core/types';
import { lobbyCardEmbed, lobbyCardComponents, type CardModel } from './render';
 ...
 */
const BOARD_STATES = ['lobby_creating', 'open', 'ready', 'in_progress'] as const;
...
interface CardState {
  ...
  /** Last rendered content, so an unchanged game costs no Discord call. */
  fingerprint: string;
  /** Serialises renders for one game — snapshots can arrive faster than Discord replies. */
  busy: Promise<void>;
}

export class LobbyBoard {
      ...
      .onSnapshot(
        (snap) => {
          for (const change of snap.docChanges()) {
            const game = change.doc.data() as InhouseGame;
        ...
        (error) => logger.error('[Discord] Lobby board listener failed', error)
      );

    logger.info('[Discord] Lobby board watching published games');
  }
  ...
  private async channel(): Promise<TextChannel | null> {
    try {
      ...
      if (!channel || channel.type !== ChannelType.GuildText) {
        logger.error('[Discord] INHOUSE_DISCORD_CHANNEL_ID is not a text channel');
        return null;
      }
      return channel as TextChannel;
    } catch (error) {
      logger.error('[Discord] Could not fetch the inhouse channel', error);
      return null;
    }
  }

  /** Post or edit the card for one game. Serialised per game. */
  private upsert(game: InhouseGame): Promise<void> {
    ...
    state.busy = state.busy
      ...
      .catch((error) => logger.error(`[Discord] Card render failed for game ${game.id}`, error));
    return state.busy;
  }

  private async render(game: InhouseGame, state: CardState): Promise<void> {
    const channel = await this.channel();
    if (!channel) return;

    const model = await this.buildModel(game);
    const embed = lobbyCardEmbed(model);
    const components = lobbyCardComponents(game);
    ...
    if (fingerprint === state.fingerprint && state.messageId) return;
    ...
    if (state.messageId) {
      try {
        ...
        logger.warn(`[Discord] Card ${state.messageId} for game ${game.id} is gone — reposting`);
    ...
    try {
      ...
      logger.error(`[Discord] Could not record the card id for game ${game.id}`, error);
    }
  }

  /** Delete the card for a game that is no longer on the board. */
  private async retire(game: InhouseGame): Promise<void> {
    const state = this.cards.get(game.id);
    ...
    try {
      ...
      logger.info(`[Discord] Removed the card for game #${game.gameNumber}`);
    } catch {
   ...
   * reported as removed.
   */
  private async removeStaleCards(): Promise<void> {
    ...
    try {
      ...
      logger.error('[Discord] Could not scan for stale cards', error);
      return;
    }

    const stale = games.filter(
      (g) => g.discord?.cardMessageId && !BOARD_STATES.includes(g.state as (typeof BOARD_STATES)[number])
    );
    for (const game of stale) await this.retire(game);
    if (stale.length) logger.info(`[Discord] Cleaned up ${stale.length} stale card(s)`);
  }

  /** Names come from memberships; counts come from the snapshot the worker writes. */
  private async buildModel(game: InhouseGame): Promise<CardModel> {
    ...
    if (seated > 0) {
      try {
        ...
        logger.warn(`[Discord] Could not read the roster for game ${game.id}`, error);
      }
    }
...
function displayNameFor(member: Membership): string {
