...
import type { InhouseStore, ModerationRecord } from './core';
...
export class BanGuard {
  private cache = new Map<string, { banned: boolean; record: ModerationRecord | null; at: number }>();
  private static readonly CACHE_TTL_MS = 60_000;

  constructor(
    private store: InhouseStore,
    private hooks: BanGuardHooks
  ) {}
   ...
   */
  async enforce(steamId32: string, playerName?: string | null): Promise<boolean> {
    let status: { banned: boolean; record: ModerationRecord | null };
    ...
    if (cached && Date.now() - cached.at < BanGuard.CACHE_TTL_MS) {
      ...
      try {
        ...
        logger.error(
          `[InhouseRunner] Ban check failed for ${steamId32} — failing open, player NOT kicked`,
          error
        );
        return false;
      }
    ...
    logger.warn(`[InhouseRunner] Banned player ${label} (${steamId32}) joined — kicking`);

    try {
      await this.hooks.kick(steamId32);
    } catch (error) {
      logger.error(`[InhouseRunner] Failed to kick banned player ${steamId32}`, error);
    }
