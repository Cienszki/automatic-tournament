...
export class InhouseRunner {
  ...
  private async reattachOrCreate(): Promise<void> {
    ...
    if (!this.lobbyCreated && !this.finalizing) {
      logger.error(
        `[InhouseRunner] No create_inhouse_lobby command arrived for game ${this.gameId} within ` +
  ...
  private async onLobbyCleared(): Promise<void> {
    ...
    if (!matchId) {
      logger.info(`[InhouseRunner] Lobby for game ${this.gameId} cleared with no match — nothing to report`);
   ...
   */
  private async finishUp(exitCode: number): Promise<void> {
    if (this.finalizing) return;
