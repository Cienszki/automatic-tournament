...
import { LobbyCommandRouter, hostLine, type LobbyMember } from './chat-commands';
...
function toSteamId64(steamId32: string | null): string | null {
  if (!steamId32 || !/^\d+$/.test(steamId32)) return null;
...
export class InhouseRunner {
  ...
  async run(): Promise<number> {
    ...
    const alreadyOver = isTerminal(this.game.state);
    ...
    if (alreadyOver) return this.closeOrphanedLobby();
   ...
   */
  private async closeOrphanedLobby(): Promise<number> {
    ...
    try {
      const acct = (await this.db.collection('botAccounts').doc(this.botAccountId).get()).data() as
        | { leasedByGameId?: string | null }
        | undefined;
      if (acct?.leasedByGameId && acct.leasedByGameId !== this.gameId) {
        logger.warn(
          `[InhouseRunner] Not closing lobby ${lobbyId}: account ${this.botAccountId} has since been ` +
      ...
      logger.warn(`[InhouseRunner] Could not verify the lease on ${this.botAccountId} — not closing`, error);
      return 1;
    }

    logger.warn(
      `[InhouseRunner] Game ${this.gameId} is '${this.game!.state}' but still owns Dota lobby ` +
    ...
    try {
      ...
      if (this.dota!.hasLobby()) {
        ...
        logger.info(`[InhouseRunner] Orphaned lobby ${lobbyId} destroyed (game ${this.gameId})`);
      } else {
        logger.info(`[InhouseRunner] Lobby ${lobbyId} is already gone from the GC — nothing to close`);
      }
      closed = true;
    } catch (error) {
      logger.error(
        `[InhouseRunner] Could not close orphaned lobby ${lobbyId} for game ${this.gameId} — it may ` +
          `still be listed in Dota's lobby browser. This is not retried; close it by hand if it is.`,
        error
      );
    }
    ...
    try {
      ...
      logger.warn(`[InhouseRunner] Could not clear dotaLobbyId on game ${this.gameId}`, error);
    }
    try {
      await releaseAccount(this.db, this.botAccountId);
    } catch (error) {
      logger.warn(`[InhouseRunner] Failed to release account ${this.botAccountId}`, error);
    }
    try {
   ...
   */
  private async lobbyMembersWithNames(): Promise<LobbyMember[]> {
   ...
   */
  private async releaseLobby(): Promise<void> {
    if (!this.dota) return;
    try {
      await this.dota.leaveLobby();
      this.lobbyReleased = true;
    } catch (error) {
      logger.warn(`[InhouseRunner] Failed to leave the lobby for game ${this.gameId}`, error);
    }
  }
   ...
   */
  private reconcileLobby(): boolean {
    if (this.finalizing || !this.lobbyCreated || !this.dota) return false;
   ...
   */
  private heldSlots(now = Date.now()): number {
   ...
   */
  private async checkLobbyLifetime(): Promise<void> {
    ...
    if (this.launching || this.countdownTimer) return;
    ...
    try {
      ...
      logger.warn(`[InhouseRunner] Could not re-read game ${this.gameId} for the idle check`, error);
      return;
    }
    ...
    // must not.
    if (empty && this.heldSlots() > 0) return;

    if (idleMs < (empty ? EMPTY_LOBBY_CLOSE_MS : SEATED_IDLE_CLOSE_MS)) return;

    const minutes = Math.round(idleMs / 60_000);
    logger.info(
      `[InhouseRunner] Closing game ${this.gameId}: ${empty ? 'empty' : `${this.playersSeated} seated, no slot activity`} ` +
        `for ${minutes} min`
    );
    ...
    await this.finishUp(
      0,
      'expired',
      empty ? `Puste lobby przez ${minutes} min` : `Brak aktywności w lobby przez ${minutes} min`
    );
  }
  ...
  private async finishUp(
    ...
    if (this.lobbyReleased) {
      try {
        ...
        logger.warn(`[InhouseRunner] Could not clear dotaLobbyId on game ${this.gameId}`, e);
      }
    }
