...
export function MatchdayCarousel({ matches }: MatchdayCarouselProps) {
    ...
    // All unique season rounds derived from match data
    ...
    // Derive available rounds and auto-select the active one
    useEffect(() => {
        if (!matches.length) {
            setRounds([]);
            return;
        ...
        for (const r of sortedRounds) {
            ...
            // If all rounds are in the past, stay on the last one
            activeRound = r;
        }
    ...
    useEffect(() => {
        ...
        const matchdayList = Object.keys(groups)
            .map(key => ({ id: parseInt(key), matches: groups[parseInt(key)] }))
            .sort((a, b) => a.id - b.id);

        setMatchdays(matchdayList);

        // Auto-jump to the first upcoming matchday within this round
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        ...
        setCurrentIndex(activeIndex !== -1 ? activeIndex : Math.max(0, matchdayList.length - 1));
        setDirection(0);
        ...
        <div className="relative w-full py-6 md:py-8">
            ...
            <div className="relative mb-8 md:mb-12">
                ...
                <div className="flex flex-col items-center justify-center">
                    {/* Round Selector Dropdown — selects the season round */}
                    <div className="mb-4 relative z-30" ref={roundDropdownRef}>
                            <button
                    ...
                    {/* Main Title — shows the current matchday (Kolejka) within the selected round */}
                    <AnimatePresence mode="wait">
                        <motion.div
                            key={`${selectedRound}-${currentIndex}`}
