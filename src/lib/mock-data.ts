...
const chartColorClasses = ['text-chart-1', 'text-chart-2', 'text-chart-3', 'text-chart-4', 'text-chart-5'];
export const heroColorMap: Record<string, string> = defaultHeroNames.reduce((acc, heroName, index) => {
  acc[heroName] = chartColorClasses[index % chartColorClasses.length];
  return acc;
...
const createTeamPlayers = (teamIndex: number, teamStatus: TournamentStatus): Player[] => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (playerSourceIndex >= mockPlayers.length) {
         // This should not happen if mockPlayers has 60 entries for 12 teams
         console.warn(`Not enough mock players to form team ${teamIndex + 1}`);
         // Create dummy players if needed, though ideally mockPlayers should be sufficient
         teamPlayersSource.push({
           id: `pdummy${i}-t${teamIndex+1}`,
           nickname: `DummyPlayer${i}`,
           mmr: 3000,
           role: PlayerRoles[i % PlayerRoles.length],
           status: teamStatus,
           steamProfileUrl: 'http://steamcommunity.com',
           profileScreenshotUrl: `https://placehold.co/600x400.png?text=DP${i}`
         });
         continue;
    }
  ...
  let currentTeamPlayers = teamPlayersSource.map((basePlayer, i) => ({
    ...basePlayer,
    id: `${basePlayer.id.split('-t')[0]}-t${teamIndex + 1}`, // Ensure base ID + team context
    role: PlayerRoles[i % PlayerRoles.length] as PlayerRole,
  ...
  return categoriesMeta.map(cat => {
    ...
    const rankings: CategoryRankingDetail[] = top5ForCategory.map((playerData, i) => {
      ...
      return {
        ...
        playerId: playerData.playerId.split('-t')[0],
    ...
    while(rankings.length < 5 && rankings.length < mockPlayers.length) {
        const fallbackPlayer = mockPlayers[rankings.length % mockPlayers.length];
